...
- (id)initWithFrame:(CGRect)frame
{
    if ((self = [super initWithFrame:CGRectMake(CGRectGetMinX(frame), CGRectGetMinY(frame), CGRectGetWidth(frame),CGRectGetHeight(frame))]))
	{
		...
		self.searchFrame.layer.cornerRadius = 0;
        ...
        self.searchFrame.layer.masksToBounds = NO;
        self.searchFrame.layer.shadowOffset = CGSizeMake(1, 1);
        self.searchFrame.layer.shadowRadius = 15;
        self.searchFrame.layer.shadowOpacity = 0.3;
		...
		self.searchField.font = [UIFont fontWithName:@"IRANSans" size:12.0];
//		self.searchField.textColor = [UIColor colorWithRed:17.0/255.0 green:190.0/255.0 blue:227.0/255.0 alpha:1.0];
        self.searchField.textColor = [UIColor colorWithRed:92.0/255.0 green:92.0/255.0 blue:92.0/255.0 alpha:1.0];
...
{
    if ([self.delegate respondsToSelector:@selector(searchBarDidTap:)])
    {
        [self.delegate searchBarDidTap:self];
    }
    return YES;
}
